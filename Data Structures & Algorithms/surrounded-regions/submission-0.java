class Solution {
    public void solve(char[][] board) {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                if (board[r][c] == 'O') dfs(r, c, directions, board, 'O', 'X');
            }
        }

        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                if (board[r][c] == '1') dfs(r, c, directions, board, '1', 'O');
            }
        }
    }
    ...
    void dfs(int r, int c, int[][] directions, char[][] board, char oldChar, char newChar) {
        ...
        if (board[r][c] == newChar || board[r][c] == 'X') {
            return;
        }

        if (board[r][c] == oldChar) board[r][c] = newChar;
